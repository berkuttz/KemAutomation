...
def cleanMailBody(rawfilw):
    ...
    StopWord = ["Freight Operative Department", "From: ", "KEMIRA GDAŃSK" ]
    ''' 
    ANALYZE THE TEXT
...
def main():
    ...
    finalfile = DataSource.path + 'ShipExprClean.xlsm'

    # load team members
    team = DataSource.iwexportteam

    # ****cleaning data
    DataFrame = cleanMailBody(
        rawfile)  # in case of Inbox only save result df2.to_excel(finalfile,sheet_name="Sent", index=False)
    df = DataFrame.to_excel(rawfile, index=False)
