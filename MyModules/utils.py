...
class global_variable:
    ...
    def printer(self):
        ...
        return "Print to PDF Document - Foxit PhantomPDF Printer"
...
class COO_automation:
    ...
    def InpoutBox(self, ShipToNr):
        ...
        def setaddress(ShipToNr, name, address):
            ...
            filename = 'C:\\Users\\' + UserLogin + '\\Kemira Oyj\\BSC CS Export - Documents\\MacroVlad\\DB\\COO accounts ' \
                                                   'and ConsgineeAddress\\COOmain.xlsx '
            xlwb = xlApp.Workbooks.Open(filename, False, False, None, Password='COO123')
    ...
    def loadExcel(self):
        xlApp = win32com.client.Dispatch("Excel.Application")
        filename = 'C:\\Users\\' + global_variable().login() + '\\Kemira Oyj\\BSC CS Export - Documents\\MacroVlad\\DB\\COO ' \
                                                               'accounts and ConsgineeAddress\\COOmain.xlsx'
        xlwb = xlApp.Workbooks.Open(filename, False, True, None, Password='COO123')
        return xlwb

    # get Invocie nr with path of the file form C:/Temp and enter it to the pop-up window
    def uploadInvoice(self):
        MyTxtFile = codecs.open(r'C:\temp\DocsStampNote2.txt', 'r', 'utf-16')
        mylist = MyTxtFile.readlines()
        mytext = mylist[0]
...
class Jenkar_automation:
    ...
    def handle_popup_Jenkar(self, text):
        i = 0
        while i < 8:
